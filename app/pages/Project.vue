<template>
    ...
    <div class="space-y-16">
      ...
      <section class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <div class="project-content absolute inset-x-0 bottom-0 p-5 bg-gradient-to-t from-black/80 via-black/60 to-transparent text-white transition-opacity duration-300 md:group-hover:opacity-0 z-10">
            <h3 class="text-xl font-semibold mb-1">{{ project.title }}</h3>
            <button @click.stop="openDetailModal(project)" class="text-purple-400 hover:text-purple-300 text-sm focus:outline-none mt-2">See More</button>
          </div>
      ...
      <section class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <div class="project-content absolute inset-x-0 bottom-0 p-5 bg-gradient-to-t from-black/80 via-black/60 to-transparent text-white transition-opacity duration-300 md:group-hover:opacity-0 z-10">
            ...
            <button @click.stop="openDetailModal(item)" class="text-purple-400 hover:text-purple-300 text-sm focus:outline-none mt-2">See More</button>
          </div>
  ...
  <Transition name="modal-fade">
    <div
      v-if="isDetailModalOpen && selectedProject"
      class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm"
      @click.self="closeDetailModal"
    >
      <div 
        class="w-full max-w-lg mx-auto bg-gray-950/95 rounded-xl border border-white/20 shadow-2xl overflow-hidden transform transition-all duration-300 ease-out"
      >
        <div class="relative">
          <img :src="selectedProject.image" :alt="selectedProject.title" class="w-full h-48 object-cover opacity-70" />
        ...
        <div class="p-5 text-gray-300 max-h-[70vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-transparent">
          ...
          <p class="text-sm leading-relaxed whitespace-pre-wrap">{{ selectedProject.description }}</p>
...
// 💡 STATE BARU UNTUK MODAL DESKRIPSI
...
const openDetailModal = (project: Project) => {
  ...
  // Nonaktifkan scrolling body saat modal terbuka
...
const uiuxProjects = ref<Project[]>([
...
/* Efek Hover (Pertahankan hanya untuk Desktop) */
@media (min-width: 768px) {
  .group:hover .project-image {
    filter: blur(5px); 
  }

  /* Jika masih ingin menggunakan efek hover pada project-content di desktop */
  .group:hover .project-content {
    opacity: 0;
  }
}

/* Hapus project-overlay lama yang berbasis hover, diganti dengan modal */
...
/* Animasi untuk konten di dalam modal */
